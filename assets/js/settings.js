...
function mapParameterMappingHandler(event) {
    ...
    newRow.querySelectorAll('select[name*="map_custom_field_id_"]').forEach(function (element) {
        ...
        element.id = 'map_custom_field_id_' + numberOfItems;
    });
    newRow.querySelectorAll('select[name*="map_consent_field_id_"]').forEach(function (element) {
        element.value = '0';
        element.name = 'map_consent_field_id_' + numberOfItems;
        element.id = 'map_consent_field_id_' + numberOfItems;
    });
    newRow.querySelectorAll('select[name*="map_group_id_"]').forEach(function (element) {
        element.value = '0';
        element.name = 'map_group_id_' + numberOfItems;
        element.id = 'map_group_id_' + numberOfItems;
    });
