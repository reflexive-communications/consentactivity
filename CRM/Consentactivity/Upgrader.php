...
class CRM_Consentactivity_Upgrader extends CRM_Extension_Upgrader_Base
{
    ...
    public function enable(): void
    {
        ...
        if (array_key_exists('tag-id', $cfg) && array_key_exists('expired-tag-id', $cfg)) {
            ...
            if ($cfg['tag-id'] !== Config::DEFAULT_TAG_ID && !Service::tagExists(intval($cfg['tag-id']))) {
                $cfg['tag-id'] = Config::DEFAULT_TAG_ID;
            }
            if ($cfg['expired-tag-id'] !== Config::DEFAULT_EXPIRED_TAG_ID && !Service::tagExists(intval($cfg['expired-tag-id']))) {
                $cfg['expired-tag-id'] = Config::DEFAULT_EXPIRED_TAG_ID;
            }
            // At this point, if the tag id is default value, delete the saved searches without thinking.
            if ($cfg['tag-id'] === Config::DEFAULT_TAG_ID || $cfg['expired-tag-id'] === Config::DEFAULT_EXPIRED_TAG_ID) {
                if (array_key_exists('saved-search-id', $cfg) && $cfg['saved-search-id'] !== Config::DEFAULT_EXPIRATION_SEARCH_ID) {
                    Service::deleteSavedSearch($cfg['saved-search-id']);
                    $cfg['saved-search-id'] = Config::DEFAULT_EXPIRATION_SEARCH_ID;
                }
                if (array_key_exists('tagging-search-id', $cfg) && $cfg['tagging-search-id'] !== Config::DEFAULT_TAG_SEARCH_ID) {
                    Service::deleteSavedSearch($cfg['tagging-search-id']);
                    $cfg['tagging-search-id'] = Config::DEFAULT_TAG_SEARCH_ID;
                }
            } else {
                // validate the search ids. Rebuild them based on the current settings..
                if (array_key_exists('saved-search-id', $cfg) && $cfg['saved-search-id'] !== Config::DEFAULT_EXPIRATION_SEARCH_ID) {
                    // check that the saved search exists
                    $currentSearch = Service::getSavedSearch($cfg['saved-search-id']);
                    if (empty($currentSearch)) {
                        $savedSearch = Service::savedSearchExpired($current['name'], $cfg['tag-id'], $cfg['expired-tag-id'], false);
                        $cfg['saved-search-id'] = $savedSearch['id'];
                    }
                }
                if (array_key_exists('tagging-search-id', $cfg) && $cfg['tagging-search-id'] !== Config::DEFAULT_TAG_SEARCH_ID) {
                    // check that the saved search exists
                    $currentSearch = Service::getSavedSearch($cfg['tagging-search-id']);
                    if (empty($currentSearch)) {
                        $savedSearch = Service::savedSearchTagging($current['name'], $cfg['expired-tag-id'], false);
    ...
    public function upgrade_3001(): bool
    {
        ...
        if (!array_key_exists('consent-expiration-years', $cfg)) {
            $cfg['consent-expiration-years'] = Config::DEFAULT_CONSENT_EXPIRATION_YEAR;
        }
        if (!array_key_exists('consent-expiration-tagging-days', $cfg)) {
            $cfg['consent-expiration-tagging-days'] = Config::DEFAULT_CONSENT_EXPIRATION_TAGGING_DAYS;
        }
    ...
    public function upgrade_3002(): bool
    {
        ...
        if (!array_key_exists('custom-field-map', $cfg)) {
            $cfg['custom-field-map'] = Config::DEFAULT_CUSTOM_FIELD_MAP;
        }
